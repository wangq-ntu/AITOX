...
def process_gene_file(filepath):
    ...
    # Use original filename to determine concentration
    ...
    logger.info(f"Assigned concentration: {concentration}")

    # Pivot the DataFrame to have gene_name as columns and log2FoldChange as values
    gene_df_pivot = gene_df.pivot(columns='gene_name', values='log2FoldChange')
    gene_df_pivot['Concentration'] = concentration
    logger.info(f"Pivoted gene_df columns: {list(gene_df_pivot.columns)}")
    logger.info(f"Pivoted gene_df shape: {gene_df_pivot.shape}")

    return gene_df_pivot, None

def prepare_features(gene_df):
    # gene_df is already pivoted with gene names as columns and Concentration
    Xv_test = gene_df.copy()
...
def index():
    if request.method == 'POST':
        ...
        logger.info(f"Scaled features sample: {Xv_test_scaled.flatten()[:10]}")
